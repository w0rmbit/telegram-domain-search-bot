...
if not BOT_TOKEN or not DATABASE_URL or CHANNEL_ID == 0:
    raise RuntimeError("Missing required env vars: BOT_TOKEN, CHANNEL_ID, DATABASE_URL")
...
try:
    ...
    """)
    # Optional: index to speed up LIKE searches
...
def index_domains_from_file(file_path: str):
    if not conn:
        print("[INDEX SKIP] No DB connection.")
        return
...
def handle_help(message):
    help_text = (
        ...
        "- Envoyez un fichier .txt (<50 MB) pour indexation.\n"
...
def search_domain(message):
    if not conn:
        bot.reply_to(message, "❌ DB indisponible.")
        return
    ...
    try:
        with conn.cursor() as c:
            c.execute("SELECT domain FROM domains WHERE domain LIKE %s LIMIT 50", (f"%{term}%",))
            rows = c.fetchall()
        if rows:
            lines = "\n".join(r[0] for r in rows)
            bot.reply_to(message, f"🔎 Résultats:\n<code>{lines}</code>")
        else:
...
def handle_document(message):
    ...
    # File size guard (avoid Telegram 400)
    if doc.file_size and doc.file_size > MAX_TG_FILE_SIZE:
        bot.send_message(
            message.chat.id,
            f"❌ Fichier trop volumineux ({doc.file_size / (1024*1024):.2f} MB). "
            "Limite Telegram: 50 MB. Uploadez sur GoFile et envoyez le lien du dossier."
        )
        return
